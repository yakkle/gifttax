...
1. 증여일 기준 전 2개월 + 1일부터 후 2개월 - 1일까지의 종가 조회 및 평균 계산
2. 해당 날짜 매매 기준 환율 조회
3. 증여금액 계산
4. 예상 증여세 계산
...
- 평균 산정 기간은 `gift_date - 2개월 + 1일` 부터 `gift_date + 2개월 - 1일` 까지다
- 외부 시세 API가 종료일 미포함 방식이면 integrations 호출 시 내부적으로만 종료일에 `+1일` 보정할 수 있다
- 사용자에게 표시하는 기간과 결과 모델의 `period_end` 는 항상 `gift_date + 2개월 - 1일` 이어야 한다
- `gift_date + 2개월` 이전에는 계산을 수행하지 않고 계산 가능일을 안내한다
